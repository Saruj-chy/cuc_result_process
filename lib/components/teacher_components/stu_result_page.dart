...
class _StudentResultPageState extends State<StudentResultPage> {
  ...
  var res_id, update_number, cq, mcq, practical, userId, currentDateTime;
  ...
  Future<void> fetchResult() async {
    ...
    setState(() {
      ...
      showProgress = false;
      itemArray = List<String>.from(jsonResponse["res_item_id"]);
    });
    ...
    if (response.statusCode == 200 && !update == false) {
      setState(() {
        ...
        if (userId != userID) {
          ToastMsg(
              "Result already inserted. You are not allowed to insert the result.");
          ...
          currentDateTime = jsonResponse["data"]["current_date_time"];

          dateDiffer(currentDateTime);
        }
      });
    }
  }

  void dateDiffer(dbCurrentDateTime) {
    String currentDateTime =
        DateFormat("yyyy-MM-dd HH:mm:ss").format(DateTime.now());

    DateTime date1 = DateTime.parse(dbCurrentDateTime);
              ...
              child: Column(
                children: <Widget>[
                  if (itemArray.isNotEmpty) ...[
                    ...
                    EditTextField(
                      textController: cqController,
                      hintText: "Enter CQ number",
                    ...
                    if (itemArray.length > 1) ...[
                      EditTextField(
                        textController: mcqController,
                        hintText: "Enter MCQ number",
                        keyBoardType: TextInputType.number,
                        isRead: isRead,
                      )
                    ],
                    ...
                    if (itemArray.length > 2) ...[
                      EditTextField(
                        textController: practicalController,
                        hintText: "Enter Practical number",
                        keyBoardType: TextInputType.number,
                        isRead: isRead,
                      )
                    ],
                        ...
                        child: MaterialButton(
                          onPressed: () {
                            if (!update) {
                              ...
                              if (!updateAccess) {
                                ToastMsg(
                                    "72 hours passed. You are not allowed to update the result.");
                              } else {
                                showDeleteDialog(context, update);
                              }
                            }
